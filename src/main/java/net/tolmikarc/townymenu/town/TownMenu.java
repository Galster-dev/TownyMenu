...
public class TownMenu extends Menu {
    ...
    public TownMenu(Town town, Player player) throws NotRegisteredException {
        ...
        if (Settings.ECONOMY_ENABLED) {
            if (TownySettings.isBankActionLimitedToBankPlots()) {
                if (TownyAPI.getInstance().getTownBlock(player.getLocation()) != null)
                    ...
                    else economyButton = new ButtonMenu(new EconomyManagementMenu(town), economyMenuItem);
                else economyButton = new Button() {
                    @Override
                    public void onClickedInMenu(Player player, Menu menu, ClickType clickType) {
                        Common.tell(player, Localization.Error.MUST_BE_IN_BANK);
                        player.closeInventory();
        ...
        else plotMenuButton = Button.makeDummy(plotMenuItem);
    ...
    public ItemStack getItemAt(int slot) {
        ...
        if (slot == 2) return toggleMenuButton.getItem();
        if (slot == 4) return residentListButton.getItem();
        if (slot == 6) return townyPermButton.getItem();

        if (slot == 9 * 2 + 2) return generalSettingsButton.getItem();
        if (slot == 9 * 2 + 4 && Settings.ECONOMY_ENABLED) return economyButton.getItem();
        if (slot == 9 * 2 + 6) return invitePlayerButton.getItem();

        if (slot == 9 * 3 + 3) return extraInfoButton.getItem();
        if (slot == 9 * 3 + 5) return plotMenuButton.getItem();
    ...
    public class ToggleSettingsMenu extends Menu {
        ...
        public ItemStack getItemAt(int slot) {
            ...
            if (slot == 1) return fireToggle.getItem();
            if (slot == 2) return mobsToggle.getItem();
            if (slot == 3) return explosionToggle.getItem();
            if (slot == 4) return pvpToggle.getItem();
            if (slot == 5) return publicToggle.getItem();
            if (slot == 6) return openToggle.getItem();
            if (slot == 7) return taxPercentToggle.getItem();
    ...
    public class ResidentMenu extends Menu {
        ...
        @Override
        public ItemStack getItemAt(int slot) {
            if (slot == 9 + 1) return kickButton.getItem();
            if (slot == 9 + 3) return titleButton.getItem();
            if (slot == 9 + 5) return rankButton.getItem();
            if (slot == 9 + 7) return mayorButton.getItem();
    ...
    public class TownyPermMenu extends Menu {
        ...
        public ItemStack getItemAt(int slot) {
            ...
            if (slot == 9 + 1) return BUILD_BUTTON;
            if (slot == 9 * 2 + 1) return BREAK_BUTTON;
            if (slot == 9 * 3 + 1) return ITEM_USE_BUTTON;
            if (slot == 9 * 4 + 1) return SWITCH_BUTTON;

            if (slot == 9 + 3) return buildResidentButton.getItem();
            if (slot == 9 + 4) return buildNationButton.getItem();
            if (slot == 9 + 5) return buildAllyButton.getItem();
            if (slot == 9 + 6) return buildOutsiderButton.getItem();

            if (slot == 9 * 2 + 3) return breakResidentButton.getItem();
            if (slot == 9 * 2 + 4) return breakNationButton.getItem();
            if (slot == 9 * 2 + 5) return breakAllyButton.getItem();
            if (slot == 9 * 2 + 6) return breakOutsiderButton.getItem();

            if (slot == 9 * 3 + 3) return itemUseResidentButton.getItem();
            if (slot == 9 * 3 + 4) return itemUseNationButton.getItem();
            if (slot == 9 * 3 + 5) return itemUseAllyButton.getItem();
            if (slot == 9 * 3 + 6) return itemUseOutsiderButton.getItem();

            if (slot == 9 * 4 + 3) return switchResidentButton.getItem();
            if (slot == 9 * 4 + 4) return switchNationButton.getItem();
            if (slot == 9 * 4 + 5) return switchAllyButton.getItem();
            if (slot == 9 * 4 + 6) return switchOutsiderButton.getItem();

            if (slot == 9 * 2 + 8) return resetButton.getItem();
            if (slot == 9 * 3 + 8) return allOnButton.getItem();
    ...
    public class EconomyManagementMenu extends Menu {
        ...
        @Override
        public ItemStack getItemAt(int slot) {
            if (slot == 1) return balanceButton;
            if (slot == 3) return depositButton.getItem();
            if (slot == 5) return withdrawButton.getItem();
            if (slot == 7) return setTaxButton.getItem();
    ...
    public class GeneralSettingsMenu extends Menu {
        ...
        protected GeneralSettingsMenu(Town town) {
            ...
            townNameButton = new Button() {
                ...
                public void onClickedInMenu(Player player, Menu menu, ClickType click) {
                    ...
                    if (town.getMayor().getName().equals(player.getName())) new TownNamePrompt(town).show(player);
                    else Common.tell(player, Localization.Error.CANNOT_CHANGE_NAME);
                }
            ...
            townBoardButton = new Button() {
                @Override
                public void onClickedInMenu(Player player, Menu menu, ClickType click) {
                    if (town.getMayor().getName().equals(player.getName())) new TownBoardPrompt(town).show(player);
                    else Common.tell(player, Localization.Error.CANNOT_CHANGE_BOARD);
                }
        ...
        @Override
        public ItemStack getItemAt(int slot) {
            if (slot == 1) return setHomeBlockButton.getItem();
            if (slot == 3) return setSpawnButton.getItem();
            if (slot == 5) return townNameButton.getItem();
            if (slot == 7) return townBoardButton.getItem();
    ...
    public class ExtraTownInfo extends Menu {
        ...
        @Override
        public ItemStack getItemAt(int slot) {
            if (slot == 2) return claimInfo;
            if (slot == 6) return extraCommands;
